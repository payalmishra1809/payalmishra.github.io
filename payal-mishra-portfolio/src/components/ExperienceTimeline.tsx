...
export const ExperienceTimeline: React.FC = () => {
  return (
    <section id="experience" className="py-24 sm:py-32 relative overflow-hidden bg-zinc-50/50 dark:bg-transparent border-t border-zinc-200/80 dark:border-zinc-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Timeline Items */}
        <div className="relative border-l-2 border-zinc-200 dark:border-zinc-800 ml-4 sm:ml-8 pl-6 sm:pl-10 space-y-10">
              ...
              <div
                className={`absolute -left-[31px] sm:-left-[49px] top-4 w-4 h-4 rounded-full border-2 transition-all duration-300 ${
                  item.isCurrent
                    ...
                    : 'border-zinc-400 bg-white dark:bg-zinc-900 group-hover:border-zinc-950 dark:group-hover:border-white'
                }`}
              />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                  <div>
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      {item.badge && (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 text-emerald-700 dark:text-emerald-300 text-xs font-semibold">
                          <TrendingUp className="w-3.5 h-3.5" />
                          {item.badge}
                        </span>
                      )}
                    </div>
                    <div className="text-sm font-semibold text-zinc-600 dark:text-zinc-400 mt-1 flex items-center gap-1.5">
                  ...
                  {/* Period Badge */}
                  <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-zinc-100 dark:bg-zinc-800 border border-zinc-200/80 dark:border-zinc-700 text-xs font-semibold text-zinc-700 dark:text-zinc-300 w-fit">
                ...
                {item.highlights && item.highlights.length > 0 && (
                  <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800 space-y-2.5">
                    {item.highlights.map((h, i) => (
                      <div key={i} className="flex items-start gap-3 text-sm sm:text-base text-zinc-600 dark:text-zinc-400 leading-relaxed font-normal">
        ...
        <div className="mt-24 pt-16 border-t border-zinc-200/80 dark:border-zinc-800">
            ...
            {EDUCATION.map((edu) => (
              <SpotlightCard key={edu.id} className="p-6 sm:p-8">
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="w-11 h-11 rounded-2xl bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white flex items-center justify-center font-bold">
