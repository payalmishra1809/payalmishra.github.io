...
export const TestimonialsSection: React.FC = () => {
  ...
  return (
    <section id="recommendations" className="py-24 sm:py-32 relative overflow-hidden bg-zinc-50/50 dark:bg-transparent border-t border-zinc-200/80 dark:border-zinc-800/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
          ...
          {/* Filter Pills */}
          <div className="flex items-center gap-1.5 p-1.5 rounded-2xl bg-zinc-100 dark:bg-[#0f121d] border border-zinc-200/80 dark:border-zinc-700/80 w-fit shadow-xs">
            <button
              type="button"
              onClick={() => setFilter('all')}
              className={`px-3.5 py-1.5 rounded-xl text-xs sm:text-sm font-semibold transition-all ${
                filter === 'all'
                  ? 'bg-white dark:bg-[#161b2c] text-zinc-950 dark:text-white shadow-xs'
            ...
            >
              <div>
                <div className="flex items-center justify-between gap-2 mb-6">
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-zinc-100 dark:bg-[#131726] border border-transparent dark:border-zinc-700/60 text-xs font-semibold text-zinc-700 dark:text-zinc-200">
                    {item.type === 'academic' ? (
                  ...
                  <Quote className="w-6 h-6 text-zinc-300 dark:text-zinc-600" />
                </div>

                <p className="text-zinc-700 dark:text-zinc-200 text-base leading-relaxed italic font-normal mb-8">
              ...
              <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800">
                ...
                <div className="text-xs font-medium text-zinc-500 dark:text-zinc-300 mt-0.5">
                  {item.role}
                </div>
              </div>
            </SpotlightCard>
