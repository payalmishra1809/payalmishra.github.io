...
export const ProjectDetailModal: React.FC<ProjectDetailModalProps> = ({ project, onClose }) => {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-3xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 sm:p-10 shadow-2xl"
      ...
      >
        ...
        <button
          ...
          className="absolute top-6 right-6 w-9 h-9 rounded-full bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-500 hover:text-zinc-950 dark:hover:text-white flex items-center justify-center transition-colors"
        >
          <X className="w-4 h-4" />
        </button>

        {/* Category & Project Number */}
        <div className="flex items-center gap-2 text-xs font-mono-code text-zinc-500 uppercase font-semibold mb-2">
        ...
        {project.metrics && (
          <div className="mb-8 p-5 sm:p-6 rounded-2xl bg-zinc-50 dark:bg-zinc-800/60 border border-zinc-200/80 dark:border-zinc-700 flex flex-wrap items-center justify-between gap-4">
            <div>
              <div className="font-mono-code text-3xl font-bold text-zinc-950 dark:text-white">
                {project.metrics.primary}
              </div>
              <div className="text-xs font-medium text-zinc-500 uppercase tracking-wider mt-0.5">
        ...
        <div className="mb-8 p-6 rounded-2xl bg-zinc-50/70 dark:bg-zinc-800/40 border border-zinc-200/80 dark:border-zinc-800">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {project.stack.map((item) => (
              <span
                key={item}
                className="px-3 py-1.5 rounded-lg bg-zinc-100 dark:bg-zinc-800 border border-zinc-200/80 dark:border-zinc-700 text-xs font-mono-code text-zinc-800 dark:text-zinc-200"
              >
                {item}
              </span>
            ))}
        ...
        <div className="flex items-center justify-between pt-6 border-t border-zinc-200/80 dark:border-zinc-800">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 rounded-full text-xs font-semibold text-zinc-600 dark:text-zinc-400 hover:text-zinc-950 dark:hover:text-white transition-colors"
          >
            Close Case Study
          </button>
